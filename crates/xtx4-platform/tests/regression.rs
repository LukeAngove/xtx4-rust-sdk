...
//   cargo run -p xtx4-sample --target x86_64-unknown-linux-gnu --features mock
//   cp /tmp/xtx4_frames/frame_*.pbm xtx4_platform/tests/golden/
// Then run: cargo test -p xtx4-platform --test regression --target x86_64-unknown-linux-gnu --features mock
...
fn run_sample() {
    ...
    let status = Command::new("cargo")
        .args(["run", "-p", "xtx4-sample", "--target", "x86_64-unknown-linux-gnu", "--features", "mock"])
        .status()
        .unwrap();
